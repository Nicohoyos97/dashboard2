...
export function NickPanel({
  ...
  const context = useMemo<PageContext>(
    () => ({ page, ...(period ? { period } : {}), ...(lineId ? { lineId } : {}) }),
    [page, period, lineId],
  );
  const suggestionKeys = lineId
    ? [...LINE_SUGGESTIONS, ...SUGGESTIONS[page].slice(0, 2)]
    : SUGGESTIONS[page];
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-1">
              {line && (
                <button
                  type="button"
                  onClick={() => selection?.setLine(null)}
                  className="text-muted-foreground hover:text-ink rounded-lg px-2 py-1 text-[12.5px] font-semibold"
                >
                  {t('clearLine')}
                </button>
              )}
