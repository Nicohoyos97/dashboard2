...
export function LineDrawer({ selected, parent, meta, onClose }: { selected: StatementNode | null; parent: StatementNode | null; meta: StatementMeta; onClose: () => void }) {
  ...
  return (
    <Dialog.Root open={node !== null} onOpenChange={(open) => { if (!open) onClose(); }}>
      <Dialog.Portal>
            ...
            <>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-muted-foreground text-[11px] font-semibold tracking-[0.12em] uppercase">{node.section ?? ''}</p>
                  <Dialog.Title className="text-ink mt-1 text-[20px] leading-tight font-bold">{node.accountName}</Dialog.Title>
                  {node.accountNumber && <p className="text-muted-foreground text-[12.5px]">{node.accountNumber}</p>}
                </div>
                <Dialog.Close asChild>
                  <button type="button" aria-label={t('close')} className="text-muted-foreground hover:bg-secondary hover:text-ink inline-flex size-9 shrink-0 items-center justify-center rounded-xl">
              ...
              <dl className="mt-6 grid grid-cols-2 gap-4">
                <div className="bg-paper rounded-xl p-4">
                  <dt className="text-muted-foreground text-[12px] font-medium">{t('colCurrent')}</dt>
                  <dd className="text-ink mt-1 text-[20px] font-bold tabular-nums">{money(node.currentCents)}</dd>
                </div>
                <div className="bg-paper rounded-xl p-4">
                  <dt className="text-muted-foreground text-[12px] font-medium">{t('colPrior')}</dt>
                  <dd className="text-ink mt-1 text-[20px] font-bold tabular-nums">{meta.hasPrior ? money(node.priorCents) : '—'}</dd>
                </div>
              </dl>
              {meta.hasPrior ? (
                node.deltaCents !== null && (
                  <p className="text-muted-foreground mt-3 text-[13.5px]">
                    {t('colChange')}: <span className="text-ink font-semibold tabular-nums">{node.deltaCents > 0 ? '+' : ''}{money(node.deltaCents)}</span>
                    {node.deltaPct !== null && <span className="ml-2 tabular-nums">({node.deltaPct > 0 ? '+' : ''}{node.deltaPct.toFixed(1)}%)</span>}
                  </p>
                )
              ) : (
              ...
              <Dialog.Description asChild>
                <div className="mt-6 text-[13.5px]">
                  ...
                  <p className="text-muted-foreground mt-1 leading-[1.5]">
                    {node.isTotal ? t('explainTotal') : node.isSection ? t('explainSection') : t('explainLine')}
                  </p>
                  {parent && (
                    <p className="text-muted-foreground mt-3">
                      {t('drawerParent')} <span className="text-ink font-medium">{parent.accountName}</span>
                    </p>
                  )}
                </div>
              </Dialog.Description>
              ...
              <div className="border-line mt-6 border-t pt-4 text-[13px]">
                <p className="text-muted-foreground">
                  {t('drawerSource')}: <span className="text-ink font-medium">{meta.source === 'firm_entry' ? t('sourceEntry') : t('sourceDocument')}</span>
                </p>
                {node.pageNumber && <p className="text-muted-foreground mt-1">{t('drawerPage', { page: node.pageNumber })}</p>}
                {meta.versionId && (
                  <a href={`/api/documents/${meta.versionId}/download`} className="text-blue mt-2 inline-block font-semibold hover:underline">
                    {t('downloadPdf')}
                  </a>
                )}
              </div>
