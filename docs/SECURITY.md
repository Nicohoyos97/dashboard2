...
- **Registration is closed. There is no self-serve sign-up**, by email or by Google: a portal account exists because a business bought a plan and the firm provisioned it (`lib/firm/onboarding.ts` → `inviteUserByEmail` + the membership, in that order). Three locks, and the first is the load-bearing one:
  1. **The Auth server refuses a walk-in.** `enable_signup = false` in `supabase/config.toml` (local) and *Authentication → Sign In / Providers → "Allow new users to sign up"* off in the cloud project. This is what closes **Google**, where the visitor arrives already authenticated by someone else. The admin API is unaffected, so invitations, `pnpm firm:admin` and `pnpm seed:demo` all still create users. Careful with the neighbouring key: `[auth.email] enable_signup` maps to GoTrue's email *provider*, and turning it off disables email **sign-in** ("Email logins are disabled") — it stays `true`.
  2. **The app offers no sign-up.** `/signup` redirects to the plans (`lib/auth/get-started.ts`), the sign-in page links there instead of to a second form, and no Server Action calls `auth.signUp` any more.
  3. **`/callback` turns away a session nobody provisioned** — `isUnprovisioned()` in `lib/auth/provisioned.ts`: an account created seconds ago with no `entity_memberships` and no `firm_memberships` row is signed out and bounced to `/signin?error=no_account`. Older accounts are never judged, so a client between businesses keeps their pending Overview and a password reset is not a lockout.
- Sessions in `SameSite=Lax`, `Secure` (production) cookies via `@supabase/ssr` (`lib/supabase/env.ts` → `COOKIE_OPTIONS`). Session refresh + route guard in `middleware.ts` / `lib/supabase/middleware.ts`. The middleware also reads the reader's language from the verified token's `user_metadata.locale` (`i18n/preference.ts`) — a preference, never a permission, so a user rewriting their own metadata gains nothing but a different UI language.
