...
> Bootstrapped 2026-09-02 from Hoyos-Baker-Dashboard v1 (`7d32144`). Phases 0–5 complete. **Phase 6 — Hardening (2026-09-04):** demo seed (`pnpm seed:demo`); an audit pass (`/code-review` high + security review) that fixed 22 defects and added migration `0009`; a time zone per business (`0010`) so "today" is the client's calendar; expense aggregation in Postgres (`0011`, `0012`); an accessibility and mobile pass (`tests/e2e/accessibility.spec.ts`, axe WCAG 2.1 AA over eleven routes) that fixed the status-colour and muted-text tokens, an ARIA-prohibited attribute and an invalid `<dl>`; the full suite green (334 unit + 47 e2e); and the final report in **`docs/DELIVERY.md`**. Three §14 criteria remain open by scope, listed in DELIVERY §6 — Cash In vs Revenue side by side (removed in the owner's redesign), disabled granularity with a reason, and a superseded-report view — along with the firm-side queue for `account_requests` and the four notification channels that have no producer. The cloud Supabase project for 2.0 is deferred until the first deploy — see `docs/ENVIRONMENTS.md`.
