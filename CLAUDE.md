...
> Bootstrapped 2026-09-02 from Hoyos-Baker-Dashboard v1 (`7d32144`). Phases 0–5 complete. **Phase 6 — Hardening (in progress, 2026-09-04):** `pnpm seed:demo` builds a "Demo" business with six months of published data; an audit pass (`/code-review` high + security review) fixed 22 defects and added migrations `0009` (the `account_requests` guard runs before insert too) — see `docs/ASSUMPTIONS.md`. Then, at the owner's direction: **`0010`** gives each business its own `timezone`, so every "today" in the portal resolves on the business's calendar rather than the server's UTC date (`todayIn()`), and **`0011`/`0012`** move the Expenses aggregation into Postgres (`portal_expense_summary`, `portal_expense_vendors`) — the page made three calls instead of materialising the whole period. Remaining per §12: accessibility pass, mobile pass, browser walkthrough of §14, final report (§13). Known scope still open, recorded in `docs/ASSUMPTIONS.md`: no firm-side queue for `account_requests`, and four of the five notification channels have no producer yet. The cloud Supabase project for 2.0 is deferred until the first deploy — see `docs/ENVIRONMENTS.md`.
