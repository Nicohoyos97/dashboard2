...
**Discounts are the third term of net sales, and a POS report is correctable by hand (2026-09-05).** `reconcileSalesReport` checked `gross − refunds = net`. July had no discounts and passed; August had $15 of them and failed by exactly $15, blocking a report whose extraction had been right the whole time. The identity is `gross − refunds − discounts = net`, and a discount the report does not print counts as zero — the safe direction, since it can only turn a pass into a failure somebody looks at, never a failure into a false pass. `correctSalesReport()` + `SalesReportForm` then give the firm what the statement editor already gave it: type the figure the document prints, the reconciliation recomputes from what is stored, and the status follows. An empty box is "not printed" and stores null, never zero. A published report refuses the edit — withdraw first, like everything else the client can see.

**One row, two documents, one provenance pointer — the sales half no longer claims it.** `tax_obligations` carries a single `document_version_id` and a single `status`, and `persistSalesReport` was writing both. So whichever of the pair processed last claimed the row, and the other document read *"Nothing was extracted from this version; publish is not possible"* — unpublishable, with its own figures sitting in that row the whole time. Seen on the real July pair: the ST-1 created the obligation with `amount_payable` 1,328.00, its confirmation number and its due date; the Clover report then overwrote the pointer and the status on its way past. The sales half now writes `taxable_sales` and `tax_collected` and nothing else; `upsertObligation` grew an insert-only argument so a sales report that arrives before any filing can still open the row with `pending_review` without stamping that status onto a filed one. The provenance of the sales figures is the `sales_reports` row, which has its own pointer.

`clearDerived` had the same shape of bug one layer down: reprocessing a filing deleted the whole obligation, sales columns included. Right while a filing was the only thing that could create one, and a way to lose a month of sales figures the moment a POS report also wrote to the row. It now deletes a row the filing produced alone and clears only the filing's columns on a shared one — verified by reverting the fix and watching the test fail.

**Three more, all consequences of the first two, all found by the owner trying to publish August.** (1) `publishBlockers` never looked at `sales_reports` — the same omission its own comment records for tax filings — so a POS report's passing reconciliation went unread, and once the obligation stopped carrying the report's version the document had nothing derived at all. (2) The obligation a sales report *opens* carried `reconciliation: null`, and a derived row with no reconciliation blocks everything pointing at it: the row blocked the very report that created it. A row with no filing on it has nothing to reconcile, which is an empty passing result, not a missing one. (3) `crossCheckSalesTax` compared sales — but `taxable_sales` is now written by the POS report itself, so it was comparing a report to its own net sales and reporting a difference of exactly refunds + discounts every month that had any ("$70.00" on August: $55 refunds, $15 discounts). There is no second opinion on sales to have, by design, so that half is gone; tax collected against tax paid stays.

**Marking a tax obligation paid, by hand (2026-09-05).** The pipeline could already do it from an uploaded payment confirmation, and most payments are an ACH transfer whose confirmation is a number in an email — no document to upload, so a settled quarter sat on the client's portal as a balance. `/admin/entities/[id]` now lists the obligations with a **Tax obligations** section: date, amount, confirmation number, method. `recordTaxPayment()` writes a `tax_payments` row and sets `status = 'paid'`, and deliberately does not clear `amount_payable` — `remainingFor()` reads `status === 'paid'` first and shows nothing owed, while the figure stays as the record of what was owed. It does not touch `document_version_id` either: the obligation belongs to the filing, and a payment is not a filing (the third time that distinction has had to be made). The payment inherits the obligation's publication, because a status that says settled with nothing behind it explains nothing, and `tax_payments` has no DELETE policy — so the firm's entry is edited in place rather than stacked.

**Where the client collects sales tax, named (2026-09-05, migration `0024`).** Turning the sales-tax module on in the provisioning form now asks where: a closed list of the 50 states, DC and Puerto Rico, then “does a city collect its own?” and the cities as chips (“City of Niles”). The answers become `tax_jurisdictions` rows — a table that had existed since 0005 with **nothing ever writing to it**, which is why the client's Sales Taxes page carried a card that read “Jurisdictions: 0”: it counted what the filings mentioned, and the extractor puts the authority a filing names into `tax_obligations.notes` as free text. The card is gone and the names are pills under the title. `tax_type` is the one column the table needed — a row is a *registration*, not a place, and `filing_frequency` is per tax, so Illinois-for-sales and Illinois-for-income are two rows — with the unique key grown to match. The codes are derived (`US-IL`, `US-IL-CITY-OF-NILES`, accents folded so “Cañón City” and “Canon City” cannot become two registrations for one place), and `tax_jurisdictions_code_shape` finally enforces the shape 0005 only wrote in a comment. The state is required by the form, by the Server Action and by the submit button the same way a DBA name is, `refineEntity` now carrying both rules; a city typed and then retracted never reaches the database, because `salesTaxRows` drops the list when the answer is “no”. Deliberately **not** done (owner's call): linking the filings themselves to a jurisdiction — obligations keep `jurisdiction_id` null, so the per-jurisdiction filter stays as it was and neither the pipeline's `upsertObligation` nor `tax_obligations_period_idx` had to move. `0024` is applied everywhere.

**The Sales Taxes page leads with the register (2026-09-05).** The point-of-sale card's grid of figures became bars — net sales, tips, tax collected, refunds, discounts — beside a net-sales area chart over the published register periods, and the pair moved above the filing cards. They replace “Collections vs payments” and “Taxable sales”, which both plotted *filing* columns: a client asking how the month went was reading tax owed, and the taxable-sales chart restated a figure the register already prints. `salesTaxSeries` went with them. The bars carry no percentages, unlike the P&L expense composition whose shape they borrow — these five are not parts of one whole (refunds and discounts are what gross lost on the way to net, tips pass through to staff, tax is held for the state), so a share would need a denominator no document prints and the client could not check. Gross sales and the amount collected stay as figures above the bars rather than becoming bars beside their own parts, and `discounts` — which had never reached the portal at all — is now read by `loadPublishedSalesReports`. The axis needed `formatPeriodCompact`: at “Aug 1, 2026 – Aug 31, 2026” per tick Recharts drew two of six labels, so a whole calendar month collapses to “Aug 2026” and everything else keeps its days.

**A sales-tax-only client gets their own Overview (2026-09-05).** The module gate hid the P&L KPIs, the income-vs-expense chart, the expense composition and the insights from a client who bought only sales tax — correctly, they have no Profit & Loss — but nothing took their place, and the *period* the Overview is built on is itself derived from published statements, so the page fell through to "No reports published yet" even for a client whose register and filings were all up to date. `SalesTaxOverview` now serves them the two facts their engagement is made of: **net sales** from the register (the same `NetSalesChart` the Sales Taxes page draws, one series, because the register is the only source of a sales figure) and **sales tax paid** as bars, one per filing period, from `taxPaidSeries()` — grouped by period rather than by tax year, since a monthly filer would otherwise see twelve months in one column, and summed across jurisdictions, since the state and the city are two filings for one month's money. A period whose filings print no payment keeps its place on the axis with a null rather than a zero bar. The gate is `sales_taxes && !bookkeeping`, which is exactly the `sales_tax` package. `OverviewShell` / `OverviewEmpty` / `OverviewCard` moved out of the page so the two Overviews cannot drift in the header.

**A point-of-sale report is the firm's paper, not the client's (2026-09-05, migration `0025`).** The register export a client sends us was landing back in their own portal under "Available reports & documents", downloadable — the one document type where the client is the author and we are the reader, offered to them as a firm deliverable. Publishing it still has to happen: publication is what makes the figures visible, and those figures are the whole Sales Taxes page and the sales-tax-only Overview. So the rule is on the document row alone, and `document_type_is_client_visible()` states it once for all three ways a client can reach a file — the row (`documents_member_select`), its bytes (`versions_member_select`) and the storage object the download route signs (`document_object_is_client_visible`). A rule in two of the three hides a tile and still serves the PDF to anyone who guesses a version id. `FIRM_ONLY_DOCUMENT_TYPES` mirrors it in TypeScript for one reason: `loadPublishedDocuments` runs as a firm user during "preview as client", where `is_firm_member()` makes RLS return everything, so the preview would show a tile the client does not have. Publishing and withdrawing one no longer notifies the client either — the bell would name a document they cannot open. Nick needs no change: he lists through the same loader and his download tool reads through the member's own client, which now returns nothing. Applied to the cloud the same day (`db push`, ledger `0001…0025` on both sides), where it hid three already-published register reports from the clients who had been able to download them.
