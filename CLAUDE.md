...
Next: `docs/DELIVERY.md` §6 — the weekly email digest (needs a provider), `StatCards`/tax-derivation tidy-ups, the 570-line demo seed. Open questions: shortening the Supabase JWT expiry from 1 hour (closes the revoked-session window `getClaims()` opens, at the cost of more refresh round trips), and the CSV export route not checking the `statements` module gate that the PDF route does.
