...
> Bootstrapped 2026-09-02 from Hoyos-Baker-Dashboard v1 (`7d32144`). Phases 0–1 complete; Checkpoint 2 approved. **Phase 2 — Admin + ingestion (2026-09-02):** clients/businesses/people + invites, guided uploader (browser → private bucket → checksum/page count/dedupe → job), the ingestion library (`lib/ingestion/`: classify → extract → hierarchy → reconcile, Anthropic structured outputs, 199 unit tests), the worker (`/api/jobs/process-documents`, Vercel Cron, dev trigger), review page with corrections + recompute, publish/unpublish/supersede + notifications, audit trail. Next per §12: **Phase 3 — Client core** (Overview, P&L, Balance Sheet, reports library, download, reminders), working autonomously — no further checkpoints. The cloud Supabase project for 2.0 is deferred until the first deploy — see `docs/ENVIRONMENTS.md`.
