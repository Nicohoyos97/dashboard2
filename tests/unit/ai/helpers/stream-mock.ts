...
export function sseFor(spec: StreamSpec): string {
  ...
  let out = event('message_start', {
    message: {
      ...
      stop_sequence: null,
      usage: {
        input_tokens: spec.inputTokens ?? 100,
        output_tokens: 1,
  ...
  if (spec.text !== undefined) {
    ...
    for (const piece of [spec.text.slice(0, middle), spec.text.slice(middle)]) {
      if (piece)
        out += event('content_block_delta', { index, delta: { type: 'text_delta', text: piece } });
    }
...
export function mockStreams(
  specs: StreamSpec[] | ((body: JsonRecord, index: number) => StreamSpec),
): JsonRecord[] {
  ...
  server.use(
    http.post(MESSAGES_URL, async ({ request }) => {
      ...
      captured.push(body);
      const spec =
        typeof specs === 'function' ? specs(body, index) : specs[Math.min(index, specs.length - 1)];
      if (!spec) throw new Error('no canned stream');
      return new HttpResponse(sseFor(spec), {
        status: 200,
